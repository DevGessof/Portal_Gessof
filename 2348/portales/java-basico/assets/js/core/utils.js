...
const Utils = {
    ...
    /**
     * Renderiza los bloques de resumen desde JSON estructurado (Estilo JSB Premium Adaptativo)
     * @param {Array} bloques - Array de bloques de resumen
     * @returns {string} HTML renderizado
     */
    renderizarResumenJSON(bloques) {
        if (!bloques || !Array.isArray(bloques)) return '<p class="text-muted">No hay resumen disponible para este video.</p>';

        let html = '';

        bloques.forEach(bloque => {
            const tipo = bloque.tipo || 'info';

            // Mapeo de clases e iconos por tipo
            let blockClass = 'summary-section';
            let icon = 'fa-book-open';
            ...
            if (blockClass === 'summary-section') {
                html += `
                    <div class="summary-section">
                        <h3><i class="fas ${icon}"></i> ${bloque.titulo}</h3>
                `;
            } else {
                html += `
                    <div class="${blockClass}">
                        <h4 style="margin-bottom: 0.5rem; color: inherit;"><i class="fas ${icon}"></i> ${bloque.titulo}</h4>
                `;
            }
            ...
            if (bloque.contenido) {
                ...
                lineas.forEach(linea => {
                    if (linea.trim()) {
                        html += `<p class="summary-paragraph">${linea.trim()}</p>`;
                    }
                });
            }
            ...
            if (bloque.items && Array.isArray(bloque.items)) {
                ...
                bloque.items.forEach(item => {
                    // Formato markdown simple para negrita (**texto**)
                    const itemFormateado = item.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
                    html += `<li>${itemFormateado}</li>`;
                });
                html += '</ul>';
            }
