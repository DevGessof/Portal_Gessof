...
const Herramientas = {
  herramientas: [
    {
      ...
      descripcion: 'Plataforma en la nube donde funciona Apigee X',
      icono: 'fa-cloud',
      url: 'https://cloud.google.com/'
    },
    {
      nombre: 'Postman',
      descripcion: 'Herramienta esencial para probar y documentar APIs REST',
      icono: 'fa-paper-plane',
      url: 'https://www.postman.com/'
    },
    {
      nombre: 'Visual Studio Code',
      descripcion: 'Editor de código recomendado para desarrollo',
      icono: 'fa-code',
      url: 'https://code.visualstudio.com/'
    },
    {
      nombre: 'gcloud CLI',
      descripcion: 'Interfaz de línea de comandos para Google Cloud',
      icono: 'fa-terminal',
      url: 'https://cloud.google.com/sdk/gcloud'
    },
    {
      nombre: 'Chrome/Firefox',
      descripcion: 'Navegadores web modernos con herramientas de desarrollo',
      icono: 'fa-globe',
      url: 'https://www.google.com/chrome/'
    },
    {
      nombre: 'Apigee Documentation',
      descripcion: 'Documentación oficial de Apigee X',
      icono: 'fa-book',
  ...
  render() {
    ...
    const html = this.herramientas.map(h => `
      <div class="module">
        <div class="module-icon"><i class="fas ${h.icono}"></i></div>
        <h3>${h.nombre}</h3>
        <p>${h.descripcion}</p>
...
const Curso = {
      ...
      return `
        <div class="video-card ${estaVisto ? 'watched' : ''}" 
          ...
          <div class="video-info">
            ...
            ${video.materiales && video.materiales.length > 0 ? `
            <div class="video-materiales" onclick="event.stopPropagation()" style="margin-top:0.6rem; display:flex; flex-wrap:wrap; gap:0.4rem;">
              ${video.materiales.map(m => `
                <a href="https://drive.google.com/uc?export=download&id=${m.idDrive}"
                   download="${m.nombre}"
                   class="btn-material-descarga"
                   title="Descargar ${m.nombre}"
                   style="display:inline-flex; align-items:center; gap:0.35rem; font-size:0.72rem; font-weight:600; padding:0.3rem 0.7rem; border-radius:6px; background:linear-gradient(135deg,var(--primary),var(--primary-dark)); color:#fff; text-decoration:none; transition:opacity 0.2s;"
  ...
  renderTodasSecciones() {
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      // Insertar sección anexa entre sección 2 y sección 3
      if (i === 2 && DRIVE_DATA.seccionAnexa) {
        html += this.renderSeccionAnexa();
      }
    }
  ...
  renderSeccionAnexa() {
    const data = DRIVE_DATA.seccionAnexa;
    if (!data) return '';

    const iconoDoc = 'fa-file-word';
    const docsHTML = data.documentos.map(doc => `
      <div class="seccion-anexa-doc-card" style="
      ...
      " onmouseover="this.style.boxShadow='0 8px 32px rgba(0,0,0,0.15)'; this.style.transform='translateY(-2px)'"
         onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
        <div style="display:flex; align-items:center; gap:0.75rem;">
          <div style="width:42px; height:42px; border-radius:10px; background:linear-gradient(135deg,#2563EB,#1D4ED8); display:flex; align-items:center; justify-content:center; flex-shrink:0;">
            <i class="fas ${iconoDoc}" style="color:#fff; font-size:1.1rem;"></i>
          </div>
          <div>
            <h4 style="margin:0; font-size:0.95rem; color:var(--text-primary); font-weight:700;">${doc.titulo}</h4>
            <span style="font-size:0.75rem; color:var(--text-muted);"><i class="fas fa-file-word" style="color:#2563EB;"></i> Documento Word</span>
          </div>
        </div>
        <p style="margin:0; font-size:0.85rem; color:var(--text-secondary); line-height:1.65;">${doc.resumen}</p>
        <div style="margin-top:auto;">
          <a href="https://drive.google.com/uc?export=download&id=${doc.idDrive}"
             download="${doc.nombre}"
             style="display:inline-flex; align-items:center; gap:0.5rem; font-size:0.82rem; font-weight:600; padding:0.55rem 1.1rem; border-radius:8px; background:linear-gradient(135deg,var(--primary),var(--primary-dark)); color:#fff; text-decoration:none; transition:opacity 0.2s;"
             onmouseover="this.style.opacity='0.85'" onmouseout="this.style.opacity='1'">
    ...
    return `
      <div class="seccion-curso" id="seccion-anexa" style="margin-top: 1.5rem;">
        <div class="seccion-header">
          <div class="seccion-title">
            <div class="seccion-icon" style="background: linear-gradient(135deg,#2563EB,#1D4ED8);"><i class="fas fa-folder-open"></i></div>
            <div>
              <h2>Documentos Operacionales</h2>
              <p class="seccion-descripcion">Documentos de apoyo para actualización, implementación y gestión de APIs</p>
              <span class="video-count-badge"><i class="fas fa-file-word"></i> ${data.documentos.length} documentos</span>
            </div>
          </div>
        </div>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.25rem; padding: 0 0.5rem;">
          ${docsHTML}
        </div>
      </div>
    `;
